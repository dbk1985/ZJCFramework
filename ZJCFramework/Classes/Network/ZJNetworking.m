...
static ZJResponseType sg_responseType = ZJResponseTypeJSON;
static ZJRequestType  sg_requestType  = ZJRequestTypePlainText;
static ZJNetworkStatus sg_networkStatus = ZJNetworkStatusReachableViaWiFi;
...
+ (void)cancelAllRequest {
  @synchronized(self) {
    [[self allTasks] enumerateObjectsUsingBlock:^(ZJSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
      if ([task isKindOfClass:[ZJSessionTask class]]) {
        [task cancel];
      }
    }];
...
+ (ZJSessionTask *)getWithUrl:(NSString *)url
                     refreshCache:(BOOL)refreshCache
                          success:(responseSuccessBlock)success
                             fail:(responseFailBlock)fail {
...
+ (ZJSessionTask *)postWithUrl:(NSString *)url
                      refreshCache:(BOOL)refreshCache
                            params:(NSDictionary *)params
                           success:(responseSuccessBlock)success
                              fail:(responseFailBlock)fail {
...
+ (ZJSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
  ...
  ZJSessionTask *session = nil;
  
  if (httpMethod == 1) {
    if (sg_cacheGet) {
      if (sg_shoulObtainLocalWhenUnconnected) {
        if (sg_networkStatus == ZJNetworkStatusNotReachable ||  sg_networkStatus == ZJNetworkStatusUnknown ) {
...
+ (ZJSessionTask *)uploadFileWithUrl:(NSString *)url
...
+ (ZJSessionTask *)uploadWithImage:(UIImage *)image
                                   url:(NSString *)url
  ...
  AFHTTPSessionManager *manager = [self manager];
  ZJSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (ZJSessionTask *)downloadWithUrl:(NSString *)url
  ...
  @synchronized (self) {
    ...
    if (sg_sharedManager == nil || sg_isBaseURLChanged) {
      ...
      switch (sg_requestType) {
        case ZJRequestTypeJSON: {
          manager.requestSerializer = [AFJSONRequestSerializer serializer];
          break;
        }
        case ZJRequestTypePlainText: {
      ...
      switch (sg_responseType) {
        case ZJResponseTypeJSON: {
          manager.responseSerializer = [AFJSONResponseSerializer serializer];
          break;
        }
        case ZJResponseTypeXML: {
          manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
          break;
        }
        case ZJResponseTypeData: {
...
+ (void)detectNetwork {
  ...
  [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
    if (status == AFNetworkReachabilityStatusNotReachable){
      sg_networkStatus = ZJNetworkStatusNotReachable;
    } else if (status == AFNetworkReachabilityStatusUnknown){
      sg_networkStatus = ZJNetworkStatusUnknown;
    } else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
      sg_networkStatus = ZJNetworkStatusReachableViaWWAN;
    } else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
      sg_networkStatus = ZJNetworkStatusReachableViaWiFi;
    }
  }];
}
...
+ (void)successResponse:(id)responseData callback:(responseSuccessBlock)success {
  if (success) {
...
+ (void)handleCallbackWithError:(NSError *)error fail:(responseFailBlock)fail {
  if ([error code] == NSURLErrorCancelled) {
