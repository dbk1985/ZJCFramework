...
typedef downloadProgressBlock getProgress;
typedef downloadProgressBlock postProgress;
...
typedef NS_ENUM(NSUInteger, ZJResponseType) {
  ZJResponseTypeJSON = 1, // 默认
  ZJResponseTypeXML  = 2, // XML
  // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
  ZJResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, ZJRequestType) {
  ZJRequestTypeJSON = 1, // 默认
  ZJRequestTypePlainText  = 2 // 普通text/html
};

typedef NS_ENUM(NSInteger, ZJNetworkStatus) {
    ZJNetworkStatusUnknown          = -1,//未知网络
    ZJNetworkStatusNotReachable     = 0,//网络无连接
    ZJNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    ZJNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
...
// 且处理，请转换成对应的子类类型
typedef NSURLSessionTask ZJSessionTask;
